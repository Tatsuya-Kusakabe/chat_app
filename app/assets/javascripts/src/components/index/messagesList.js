...
class MessagesList extends React.Component {
  ...
  render() {
    ...
    const timestamp_partner = (openRelationship.applicant_id === openUserID)
      ? openRelationship.timestamp_applicant
      : openRelationship.timestamp_recipient
    ...
    const messagesList = openMessagesWoHash.map((message, index) => {
      ...
      const contentClasses = classNames({
        ...
        'messages-list__picture': message.pic_path !== null,
        'messages-list__content__from-current': message.sent_from === currentUserID,
        'messages-list__content__from-partner': message.sent_from !== currentUserID,
      })

      // Returing each 'message.contents'
      if (message.pic_path !== null) {
        return (
