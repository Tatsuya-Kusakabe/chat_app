...
# Responsible for 'sign up' and 'edit profile'
class Users::RegistrationsController < Devise::RegistrationsController

  # Blocking authenticated users from '/users/sign_up'
  before_action :block_authenticated_user, only: %i[new create]
  ...
  before_action      :block_unauthenticated_user, only: %i[edit update destroy]
  skip_before_action :authenticate_scope!,        only: %i[edit update destroy]
  ...
  private
    ...
    def create_params
      params.require(:user).permit(
        :name, :email, :profile_picture, :password, :password_confirmation
      )
    end
    ...
    def update_params
      create_params.reject { |_, v| v.blank? }
    end
