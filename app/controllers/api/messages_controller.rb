...
class Api::MessagesController < ApplicationController
  ...
  def index(friend_bln)
    get_messages(friend_bln, search_bln = false)
  end
  #
  def show(friend_bln)
    get_messages(friend_bln, search_bln = true)
  end
  #
  def create
    #
  ...
  private
    #
    def get_messages(friend_bln, search_bln)
      #
      # Defining '@messages'
      #
      @messages = []
      ...
      while num_id < (itr_fin + 1) do
        ...
        #
        # If calling 'SEARCH_FRIENDS' or 'SEARCH_SUGGESTIONS'
        #
        if (!!search_bln)
          #
